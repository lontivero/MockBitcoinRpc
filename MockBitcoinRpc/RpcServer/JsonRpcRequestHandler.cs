...
using MockBitcoinRpc.JsonConverters;
...
namespace MockBitcoinRpc
{
	...
	public class JsonRpcRequestHandler<TService>
	{
		...
		private async Task<string> HandleRequestAsync(JsonRpcRequest jsonRpcRequest, CancellationToken cancellationToken)
		{
			...
			{
				var code = e.InnerException switch
				{
					ArgumentException _ => JsonRpcErrorCodes.InvalidParameter,
					RpcException rpcExc => rpcExc.Code,
					_ => JsonRpcErrorCodes.InternalError
				};
				return Error(code, e.InnerException.Message, jsonRpcRequest.Id);
			}
			catch (Exception e)
			{
